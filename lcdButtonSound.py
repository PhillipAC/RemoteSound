...
while not done:
    ...
    if not playingMusic and musicChannel.get_busy():
		...
		LCD.print_message("", LCD.LINE_1)
		LCD.print_message("", LCD.LINE_2)
		musicChannel.stop()
	...
	if not musicChannel.get_busy():
		...
		# Load the next song and start playing it
			...
			LCD.print_message("Playing Song:", LCD.LINE_1)
			LCD.print_message(musicLibrary[currentSongIndex-1], LCD.LINE_2)
			musicChannel.play()
